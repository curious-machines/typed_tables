"""Tests for the unified graph2 command (legacy graph preserved during TTGE development)."""
...
class TestGraphTable:
    ...
    def test_graph_all(self, executor, parser):
        ...
        result = _run(executor, parser, 'graph2')
    ...
    def test_graph_specific_type(self, executor, parser):
        self._setup_schema(executor, parser)
        result = _run(executor, parser, 'graph2 Person')
        edges = _edges(result)
    ...
    def test_graph_alias_edges(self, executor, parser):
        self._setup_schema(executor, parser)
        result = _run(executor, parser, 'graph2 myid')
        edges = _edges(result)
    ...
    def test_graph_enum_edges(self, executor, parser):
        self._setup_schema(executor, parser)
        result = _run(executor, parser, 'graph2 Shape')
        edges = _edges(result)
    ...
    def test_graph_array_type_filter_includes_referrers(self, executor, parser):
        ...
        result = _run(executor, parser, 'graph2 Point')
        edges = _edges(result)
    ...
    def test_graph_nonexistent_type(self, executor, parser):
        ...
        result = _run(executor, parser, 'graph2 Nonexistent')
        assert result.rows == []
    ...
    def test_graph_sort_by_source(self, executor, parser):
        ...
        result = _run(executor, parser, 'graph2 sort by source')
...
class TestGraphFileOutput:
    ...
    def test_graph_ttq(self, executor, parser):
        self._setup_schema(executor, parser)
        result = _run(executor, parser, 'graph2 > "types.ttq"')
...
class TestInheritanceEdges:
    ...
    def test_filter_expands_to_interface_edges(self, executor, parser):
        ...
        result = _run(executor, parser, 'graph2 Circle')
        edges = _edges(result)
        # Circle's own edges
        assert ("Circle", "Composite", "r", "float32") in edges
    ...
    def test_filter_expands_to_parent_edges(self, executor, parser):
        ...
        _run(executor, parser, 'type Person { name: string, age: uint8 }')
        _run(executor, parser, 'type Employee from Person { dept: string }')
        result = _run(executor, parser, 'graph2 Employee')
        edges = _edges(result)
    ...
    def test_recursive_expansion_deep_chain(self, executor, parser):
        ...
        result = _run(executor, parser, 'graph2 C')
        edges = _edges(result)
        # C's own
...
class TestInterfaceInheritanceReferences:
    ...
    def test_graph_extends_edge(self, executor, parser):
        _run(executor, parser, "interface A { x: uint8 }")
        _run(executor, parser, "interface B from A { y: uint16 }")
        result = _run(executor, parser, "graph2 B")
        edges = _edges(result)
        # B→A with (extends)
    ...
    def test_graph_inherited_fields_on_parent(self, executor, parser):
        _run(executor, parser, "interface A { x: uint8 }")
        _run(executor, parser, "interface B from A { y: uint16 }")
        result = _run(executor, parser, "graph2 A")
        edges = _edges(result)
        # A has x→uint8
...
class TestStructureView:
    ...
    def test_structure_no_focus(self, executor, parser):
        ...
        result = _run(executor, parser, 'graph2 structure')
        assert isinstance(result, QueryResult)
        edges = _edges(result)
    ...
    def test_structure_with_focus(self, executor, parser):
        ...
        _setup_boss_schema(executor, parser)
        result = _run(executor, parser, 'graph2 Boss structure')
        edges = _edges(result)
...
class TestDeclaredView:
    ...
    def test_declared_requires_focus(self, executor, parser):
        ...
        _setup_boss_schema(executor, parser)
        result = _run(executor, parser, 'graph2 declared')
    ...
    def test_declared_own_fields(self, executor, parser):
        ...
        _setup_boss_schema(executor, parser)
        result = _run(executor, parser, 'graph2 Boss declared')
        assert isinstance(result, QueryResult)
    ...
    def test_declared_creature_fields(self, executor, parser):
        ...
        _setup_boss_schema(executor, parser)
        result = _run(executor, parser, 'graph2 Creature declared')
        fields = {e["field"] for e in result.rows}
...
class TestStoredView:
    ...
    def test_stored_origin(self, executor, parser):
        ...
        _setup_boss_schema(executor, parser)
        result = _run(executor, parser, 'graph2 Boss stored origin')
    ...
    def test_stored_origin_depth_0(self, executor, parser):
        ...
        _setup_boss_schema(executor, parser)
        result = _run(executor, parser, 'graph2 Boss stored origin depth 0')
        assert result.rows == []
    ...
    def test_stored_origin_depth_1(self, executor, parser):
        ...
        result = _run(executor, parser, 'graph2 Monster stored origin depth 1')
        assert "origin" in result.columns
    ...
    def test_stored_origin_depth_2(self, executor, parser):
        ...
        _run(executor, parser, 'alias health = int16')
        _run(executor, parser, 'type Monster from Creature { shield: health }')
        result = _run(executor, parser, 'graph2 Monster stored origin depth 2')
        # Field edges have origins
        origin_map = {r["field"]: r["origin"] for r in result.rows if "origin" in r}
        assert origin_map.get("shield") == "Monster"
        # Alias edge expanded
...
class TestViewModeValidation:
    ...
    def test_declared_unknown_type(self, executor, parser):
        result = _run(executor, parser, 'graph2 Nonexistent declared')
        assert result.rows == []

    def test_stored_unknown_type(self, executor, parser):
        result = _run(executor, parser, 'graph2 Nonexistent stored')
        assert result.rows == []
...
class TestDepthParser:
    def test_parse_depth(self, parser):
        q = parser.parse("graph2 Boss depth 2")
        assert isinstance(q, GraphQuery)
        assert q.focus_type == ["Boss"]
        assert q.depth == 2

    def test_parse_depth_zero(self, parser):
        q = parser.parse("graph2 Boss depth 0")
        assert isinstance(q, GraphQuery)
        assert q.depth == 0

    def test_parse_depth_to_file(self, parser):
        q = parser.parse('graph2 Boss depth 1 > "out.dot"')
        assert isinstance(q, GraphQuery)
        assert q.depth == 1
        assert q.output_file == "out.dot"

    def test_parse_depth_sort(self, parser):
        q = parser.parse("graph2 Boss depth 2 sort by source")
        assert isinstance(q, GraphQuery)
        assert q.depth == 2
        assert q.sort_by == ["source"]

    def test_parse_structure_depth(self, parser):
        q = parser.parse("graph2 Boss structure depth 1")
        assert isinstance(q, GraphQuery)
        assert q.view_mode == "structure"
        assert q.depth == 1
...
class TestDepthControl:
    def test_depth_zero_focus_only(self, executor, parser):
        ...
        _setup_boss_schema(executor, parser)
        result = _run(executor, parser, 'graph2 Boss depth 0')
        edges = _edges(result)
        assert len(edges) == 0
    ...
    def test_depth_zero_dot_shows_focus_node(self, executor, parser, tmp_data_dir):
        ...
        _setup_boss_schema(executor, parser)
        result = _run(executor, parser, f'graph2 Boss depth 0 > "{tmp_data_dir}/d0.dot"')
        assert isinstance(result, DumpResult)
    ...
    def test_depth_one_direct_edges(self, executor, parser):
        ...
        _setup_boss_schema(executor, parser)
        result = _run(executor, parser, 'graph2 Boss depth 1')
        edges = _edges(result)
    ...
    def test_depth_two(self, executor, parser):
        ...
        _setup_boss_schema(executor, parser)
        result = _run(executor, parser, 'graph2 Boss depth 2')
        edges = _edges(result)
        sources = {e[0] for e in edges}
        assert "Boss" in sources
    ...
    def test_depth_unlimited_default(self, executor, parser):
        ...
        _setup_boss_schema(executor, parser)
        result = _run(executor, parser, 'graph2 Boss')
        edges = _edges(result)
        sources = {e[0] for e in edges}
    ...
    def test_depth_structure(self, executor, parser):
        ...
        _setup_boss_schema(executor, parser)
        result = _run(executor, parser, 'graph2 Boss structure depth 1')
        edges = _edges(result)
    ...
    def test_declared_depth_0(self, executor, parser):
        ...
        _setup_boss_schema(executor, parser)
        result = _run(executor, parser, 'graph2 Creature declared depth 0')
        assert result.rows == []
    ...
    def test_declared_depth_1_no_alias_expansion(self, executor, parser):
        ...
        result = _run(executor, parser, 'graph2 Monster declared depth 1')
        fields = {e["field"] for e in result.rows}
        assert fields == {"hp"}
    ...
    def test_declared_depth_2_expands_aliases(self, executor, parser):
        ...
        _run(executor, parser, 'alias health = int16')
        _run(executor, parser, 'type Monster { hp: health }')
        result = _run(executor, parser, 'graph2 Monster declared depth 2')
    ...
    def test_declared_no_depth_full_alias_resolution(self, executor, parser):
        ...
        _run(executor, parser, 'type Monster { hp: health }')
        result = _run(executor, parser, 'graph2 Monster declared')
    ...
    def test_stored_depth_0(self, executor, parser):
        ...
        _setup_boss_schema(executor, parser)
        result = _run(executor, parser, 'graph2 Boss stored depth 0')
        assert result.rows == []
    ...
    def test_stored_depth_1_no_alias_expansion(self, executor, parser):
        ...
        _setup_boss_schema(executor, parser)
        result = _run(executor, parser, 'graph2 Boss stored depth 1')
    ...
    def test_stored_depth_2_expands_aliases(self, executor, parser):
        ...
        _run(executor, parser, 'alias health = int16')
        _run(executor, parser, 'type Monster from Creature { shield: health }')
        result = _run(executor, parser, 'graph2 Monster stored depth 2')
        alias_edges = [e for e in result.rows if e["field"] == "(alias)"]
        assert any(e["source"] == "health" and e["target"] == "int16" for e in alias_edges)
...
class TestFilterParser:
    ...
    def test_parse_showing_type(self, parser):
        r = parser.parse('graph2 showing type Person')
    ...
    def test_parse_excluding_field(self, parser):
        r = parser.parse('graph2 excluding field name')
        assert isinstance(r, GraphQuery)
    ...
    def test_parse_showing_kind(self, parser):
        r = parser.parse('graph2 showing kind Composite')
        assert isinstance(r, GraphQuery)
    ...
    def test_parse_showing_kind_interface(self, parser):
        ...
        r = parser.parse('graph2 showing kind interface')
        assert r.showing[0].dimension == "kind"
        assert r.showing[0].values == ["interface"]

    def test_parse_multi_value_brackets(self, parser):
        r = parser.parse('graph2 showing type [Person, Employee]')
    ...
    def test_parse_multi_filter(self, parser):
        ...
        r = parser.parse('graph2 showing type Person field name')
        assert len(r.showing) == 2
        assert r.showing[0].dimension == "type"
        assert r.showing[1].dimension == "field"

    def test_parse_showing_and_excluding(self, parser):
        r = parser.parse('graph2 showing type uint32 excluding kind Primitive')
        assert len(r.showing) == 1
        assert len(r.excluding) == 1
        assert r.showing[0].dimension == "type"
...
class TestShowingFilter:
    ...
    def test_showing_type(self, executor, parser):
        ...
        _setup_boss_schema(executor, parser)
        result = _run(executor, parser, 'graph2 Boss showing type uint8')
        edges = _edges(result)
    ...
    def test_showing_type_with_structural_path(self, executor, parser):
        ...
        _setup_boss_schema(executor, parser)
        result = _run(executor, parser, 'graph2 Boss showing type int16')
        edges = _edges(result)
    ...
    def test_showing_type_multiple_values(self, executor, parser):
        ...
        _setup_boss_schema(executor, parser)
        result = _run(executor, parser, 'graph2 Boss showing type [uint8, int16]')
        edges = _edges(result)
        field_edges = [e for e in edges if not e[2].startswith("(")]
    ...
    def test_showing_field(self, executor, parser):
        ...
        _setup_boss_schema(executor, parser)
        result = _run(executor, parser, 'graph2 Boss showing field phase')
        edges = _edges(result)
        field_edges = [e for e in edges if not e[2].startswith("(")]
    ...
    def test_showing_field_inherited(self, executor, parser):
        ...
        _setup_boss_schema(executor, parser)
        result = _run(executor, parser, 'graph2 Boss showing field hp')
        edges = _edges(result)
        field_edges = [e for e in edges if not e[2].startswith("(")]
        assert len(field_edges) == 1
    ...
    def test_showing_kind(self, executor, parser):
        ...
        _setup_boss_schema(executor, parser)
        result = _run(executor, parser, 'graph2 Boss showing kind Interface')
        edges = _edges(result)
    ...
    def test_showing_kind_case_insensitive(self, executor, parser):
        ...
        _setup_boss_schema(executor, parser)
        result1 = _run(executor, parser, 'graph2 Boss showing kind interface')
        result2 = _run(executor, parser, 'graph2 Boss showing kind Interface')
    ...
    def test_showing_no_match(self, executor, parser):
        ...
        _setup_boss_schema(executor, parser)
        result = _run(executor, parser, 'graph2 Boss showing type float64')
        assert len(result.rows) == 0

    def test_showing_with_structure_view(self, executor, parser):
        """showing works with structure view mode."""
        _setup_boss_schema(executor, parser)
        result = _run(executor, parser, 'graph2 Boss structure showing type NPC')
        edges = _edges(result)
...
class TestExcludingFilter:
    ...
    def test_excluding_type(self, executor, parser):
        ...
        _setup_boss_schema(executor, parser)
        result = _run(executor, parser, 'graph2 Boss excluding type string')
        edges = _edges(result)
        # No string targets
    ...
    def test_excluding_field(self, executor, parser):
        ...
        _setup_boss_schema(executor, parser)
        result = _run(executor, parser, 'graph2 Boss excluding field dialogue')
        edges = _edges(result)
    ...
    def test_excluding_kind(self, executor, parser):
        ...
        _setup_boss_schema(executor, parser)
        result = _run(executor, parser, 'graph2 Boss excluding kind Interface')
        edges = _edges(result)
    ...
    def test_excluding_multiple_values(self, executor, parser):
        ...
        _setup_boss_schema(executor, parser)
        result = _run(executor, parser, 'graph2 Boss excluding type [string, float32]')
        edges = _edges(result)
...
class TestCombinedFilters:
    ...
    def test_showing_then_excluding(self, executor, parser):
        ...
        result = _run(executor, parser, 'graph2 Boss showing type float32 excluding field speed')
        edges = _edges(result)
        field_edges = [e for e in edges if not e[2].startswith("(")]
    ...
    def test_filter_with_depth(self, executor, parser):
        ...
        result = _run(executor, parser, 'graph2 Boss depth 1 showing type uint8')
        edges = _edges(result)
        field_edges = [e for e in edges if not e[2].startswith("(")]
        assert len(field_edges) == 1
        assert field_edges[0] == ("Boss", "Composite", "phase", "uint8")
...
class TestTitleOutput:
    ...
    def test_dot_title(self, executor, parser, tmp_data_dir):
        ...
        result = _run(executor, parser, 'graph2{"title": "My Schema"} > "out.dot"')
        assert isinstance(result, DumpResult)
    ...
    def test_dot_default_title(self, executor, parser, tmp_data_dir):
        ...
        _run(executor, parser, 'type Foo { x: uint32 }')
        result = _run(executor, parser, 'graph2 > "out.dot"')
        assert isinstance(result, DumpResult)
        assert "labelloc=t" in result.script
    ...
    def test_ttq_title(self, executor, parser, tmp_data_dir):
        ...
        _run(executor, parser, 'type Foo { x: uint32 }')
        result = _run(executor, parser, 'graph2{"title": "My Schema"} > "out.ttq"')
        assert isinstance(result, DumpResult)
    ...
    def test_ttq_default_comment(self, executor, parser, tmp_data_dir):
        ...
        _run(executor, parser, 'type Foo { x: uint32 }')
        result = _run(executor, parser, 'graph2 > "out.ttq"')
        assert isinstance(result, DumpResult)
...
class TestStyleOutput:
    ...
    def test_style_direction(self, executor, parser, tmp_data_dir):
        ...
        _run(executor, parser, 'type Foo { x: uint32 }')
        result = _run(executor, parser, f'graph2{{"style": "{style_path}"}} > "out.dot"')
        assert isinstance(result, DumpResult)
    ...
    def test_style_focus_color(self, executor, parser, tmp_data_dir):
        ...
        _run(executor, parser, 'type Foo { x: uint32 }')
        result = _run(executor, parser, f'graph2{{"style": "{style_path}"}} Foo > "out.dot"')
        assert isinstance(result, DumpResult)
    ...
    def test_style_missing_file(self, executor, parser, tmp_data_dir):
        ...
        _run(executor, parser, 'type Foo { x: uint32 }')
        result = _run(executor, parser, 'graph2{"style": "nonexistent.txt"} > "out.dot"')
        assert isinstance(result, DumpResult)
    ...
    def test_title_and_style_combined(self, executor, parser, tmp_data_dir):
        ...
        style_path.write_text('{"direction": "TB"}')
        _run(executor, parser, 'type Foo { x: uint32 }')
        result = _run(executor, parser, f'graph2{{"title": "My Graph", "style": "{style_path}"}} > "out.dot"')
        assert isinstance(result, DumpResult)
    ...
    def test_inline_direction(self, executor, parser, tmp_data_dir):
        ...
        _run(executor, parser, 'type Foo { x: uint32 }')
        result = _run(executor, parser, 'graph2{"direction": "TB"} > "out.dot"')
        assert isinstance(result, DumpResult)
        assert "rankdir=TB" in result.script
    ...
    def test_inline_color(self, executor, parser, tmp_data_dir):
        ...
        _run(executor, parser, 'type Foo { x: uint32 }')
        result = _run(executor, parser, 'graph2{"composite.color": "#FF0000"} > "out.dot"')
        assert isinstance(result, DumpResult)
        assert "#FF0000" in result.script
    ...
    def test_style_file_then_inline_override(self, executor, parser, tmp_data_dir):
        ...
        _run(executor, parser, 'type Foo { x: uint32 }')
        result = _run(executor, parser, f'graph2{{"style": "{style_path}", "direction": "TB"}} > "out.dot"')
        assert isinstance(result, DumpResult)
        assert "rankdir=TB" in result.script
    ...
    def test_inline_then_style_file_override(self, executor, parser, tmp_data_dir):
        ...
        style_path.write_text('{"direction": "TB"}')
        _run(executor, parser, 'type Foo { x: uint32 }')
        result = _run(executor, parser, f'graph2{{"direction": "LR", "style": "{style_path}"}} > "out.dot"')
        assert isinstance(result, DumpResult)
        assert "rankdir=TB" in result.script
...
class TestPathToExecution:
    ...
    def test_path_to_immediate_parent(self, executor, parser):
        ...
        _setup_boss_schema(executor, parser)
        result = _run(executor, parser, 'graph2 Boss to NPC')
        edges = _edges(result)
        # Linear path edge
        assert ("Boss", "Composite", "(extends)", "NPC") in edges
    ...
    def test_path_to_grandparent(self, executor, parser):
        ...
        _setup_boss_schema(executor, parser)
        result = _run(executor, parser, 'graph2 Boss to Creature')
        edges = _edges(result)
        # Linear path edges
        assert ("Boss", "Composite", "(extends)", "NPC") in edges
        assert ("NPC", "Composite", "(extends)", "Creature") in edges
    ...
    def test_path_to_interface(self, executor, parser):
        ...
        _setup_boss_schema(executor, parser)
        result = _run(executor, parser, 'graph2 Boss to Entity')
        edges = _edges(result)
    ...
    def test_path_to_multiple_targets(self, executor, parser):
        ...
        _setup_boss_schema(executor, parser)
        result = _run(executor, parser, 'graph2 Boss to [NPC, Entity]')
        edges = _edges(result)
    ...
    def test_path_to_deep_interface(self, executor, parser):
        ...
        _setup_boss_schema(executor, parser)
        result = _run(executor, parser, 'graph2 Boss to Identifiable')
        edges = _edges(result)
    ...
    def test_path_to_unknown_target(self, executor, parser):
        ...
        _setup_boss_schema(executor, parser)
        result = _run(executor, parser, 'graph2 Boss to Unknown')
    ...
    def test_path_to_no_path(self, executor, parser):
        ...
        result = _run(executor, parser, 'graph2 Boss to Unrelated')
    ...
    def test_path_to_multiple_errors(self, executor, parser):
        ...
        _setup_boss_schema(executor, parser)
        _run(executor, parser, 'type Unrelated { x: uint32 }')
        result = _run(executor, parser, 'graph2 Boss to [Unrelated, Unknown]')
    ...
    def test_path_to_requires_focus(self, executor, parser):
        ...
        _setup_boss_schema(executor, parser)
        result = _run(executor, parser, 'graph2 to NPC')
        assert "requires a focus type" in result.message

    def test_path_to_dot_output(self, executor, parser, tmp_data_dir):
        """Path-to with DOT file output."""
        _setup_boss_schema(executor, parser)
        result = _run(executor, parser, 'graph2 Boss to Creature > "path.dot"')
        assert isinstance(result, DumpResult)
        assert "Boss" in result.script
...
class TestMultiFocusExecution:
    ...
    def test_multi_focus_full_view(self, executor, parser):
        ...
        _run(executor, parser, 'alias health = int16')
        _run(executor, parser, 'type Monster from Creature { shield: health }')
        result = _run(executor, parser, 'graph2 [Boss, Monster]')
        edges = _edges(result)
        # Boss edges
        assert ("Boss", "Composite", "phase", "uint8") in edges
        assert ("Boss", "Composite", "(extends)", "NPC") in edges
    ...
    def test_multi_focus_declared(self, executor, parser):
        ...
        _run(executor, parser, 'alias health = int16')
        _run(executor, parser, 'type Monster from Creature { shield: health }')
        result = _run(executor, parser, 'graph2 [Boss, Monster] declared')
        edges = _edges(result)
    ...
    def test_multi_focus_stored(self, executor, parser):
        ...
        _run(executor, parser, 'alias health = int16')
        _run(executor, parser, 'type Monster from Creature { shield: health }')
        result = _run(executor, parser, 'graph2 [Boss, Monster] stored')
        edges = _edges(result)
    ...
    def test_multi_focus_structure(self, executor, parser):
        ...
        _run(executor, parser, 'alias health = int16')
        _run(executor, parser, 'type Monster from Creature { shield: health }')
        result = _run(executor, parser, 'graph2 [Boss, Monster] structure')
        edges = _edges(result)
        # Boss chain
        assert ("Boss", "Composite", "(extends)", "NPC") in edges
        # Monster chain
    ...
    def test_multi_focus_dedup(self, executor, parser):
        ...
        _setup_boss_schema(executor, parser)
        result = _run(executor, parser, 'graph2 [Boss, NPC] structure')
    ...
    def test_multi_focus_dot_output(self, executor, parser):
        ...
        _setup_boss_schema(executor, parser)
        result = _run(executor, parser, 'graph2 [Boss, NPC] > "out.dot"')
        assert isinstance(result, DumpResult)
    ...
    def test_multi_focus_depth_zero(self, executor, parser):
        ...
        _setup_boss_schema(executor, parser)
        result = _run(executor, parser, 'graph2 [Boss, NPC] depth 0')
        assert "Boss" in result.message
...
class TestFocusKindExecution:
    ...
    def test_all_interfaces(self, executor, parser):
        ...
        _setup_boss_schema(executor, parser)
        result = _run(executor, parser, 'graph2 all Interfaces')
        edges = _edges(result)
    ...
    def test_all_composites(self, executor, parser):
        ...
        _setup_boss_schema(executor, parser)
        result = _run(executor, parser, 'graph2 all Composites')
        edges = _edges(result)
        assert ("Boss", "Composite", "(extends)", "NPC") in edges
        assert ("Boss", "Composite", "phase", "uint8") in edges
    ...
    def test_all_aliases(self, executor, parser):
        ...
        _run(executor, parser, 'alias health = int16')
        _run(executor, parser, 'type Monster { hp: health }')
        result = _run(executor, parser, 'graph2 all Aliases')
        edges = _edges(result)
    ...
    def test_all_enums_empty(self, executor, parser):
        ...
        result = _run(executor, parser, 'graph2 all Enums')
    ...
    def test_singleton_rejected(self, executor, parser):
        ...
        result = _run(executor, parser, 'graph2 all String')
    ...
    def test_unknown_kind(self, executor, parser):
        ...
        result = _run(executor, parser, 'graph2 all Bogus')
    ...
    def test_showing_kind_backward_walk(self, executor, parser):
        ...
        _setup_boss_schema(executor, parser)
        result = _run(executor, parser, 'graph2 showing kind Interface')
        edges = _edges(result)
    ...
    def test_showing_kind_case_insensitive(self, executor, parser):
        ...
        _setup_boss_schema(executor, parser)
        r1 = _run(executor, parser, 'graph2 showing kind interface')
        r2 = _run(executor, parser, 'graph2 showing kind Interface')
